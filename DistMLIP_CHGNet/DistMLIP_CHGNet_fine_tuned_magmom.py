...
from magmom_outcar import magmom_outcar
...
for folder in folder_names:
    ...
    try:
        ...
        magmom = magmom_outcar(outcar_path)  # list[float] or None

        if magmom is None:
            print(f"[SKIP] magmom is None: {vasp_dir}")
            continue

        if len(magmom) != len(structure):
            print(f"[SKIP] magmom len {len(magmom)} != n_atoms {len(structure)}: {vasp_dir}")
            continue
...
labels = {
    ...
    "magmoms":  [[[float(x)] for x in m] for m in magmom_list],  # (N,1)
}
...
lit_module_finetune = PotentialLightningModule(
    model=model_pretrained, element_refs=property_offset, lr=1e-3, include_line_graph=True
)
lit_module_finetune.model.calc_magmom = True
